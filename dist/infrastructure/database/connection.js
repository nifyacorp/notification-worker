...
export class DatabaseConnection {
    ...
    async initializePool() {
        ...
        try {
            ...
            const client = await Promise.race([
                newPool.connect(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Connection timeout during pool initialization')), 15000))
            ]);
     ...
     */
    async withRLSContext(userId, callback) {
        if (!this.pool) {
            this.pool = await this.initializePool();
        }
        const client = await this.pool.connect();
