...
import { ProcessorMessage, ProcessorType, BOEDocument } from '../../domain/models/message';
import { 
  Notification, 
 ...
 */
export class BOEProcessor implements MessageProcessor {
  public readonly processorType = ProcessorType.BOE;
    ...
    try {
      ...
      for (const match of validatedMessage.results.matches) {
        for (const doc of match.documents) {
          ...
          const boeDoc = doc as BOEDocument;
      ...
      throw new ProcessorError(
        `Failed to process BOE message: ${(error as Error).message}`,
  ...
  public async transform(message: ProcessorMessage): Promise<ProcessorMessage> {
    ...
    if (!transformedMessage.results.matches || !Array.isArray(transformedMessage.results.matches)) {
      ...
      if (Array.isArray((transformedMessage.results as any)?.results?.[0]?.matches)) {
        this.logger.warn('Found matches in legacy location: results.results[0].matches', {
          trace_id: traceId,
          match_count: (transformedMessage.results as any).results[0].matches.length
        });
        transformedMessage.results.matches = (transformedMessage.results as any).results[0].matches;
      } 
      // RECOVERY STRATEGY 2: Check for multiple results objects
      else if (Array.isArray((transformedMessage.results as any)?.results)) {
        const extractedMatches: any[] = [];
        
        // Try to extract all matches from all results
        (transformedMessage.results as any).results.forEach((result: any, index: number) => {
      ...
      // RECOVERY STRATEGY 3: Check if results is directly an array of matches
        ...
        this.logger.warn('Treating results.results as direct matches array', {
          trace_id: traceId,
          items_count: (transformedMessage.results as any).results.length
        });
        transformedMessage.results.matches = (transformedMessage.results as any).results;
      }
   ...
   */
  private determineEntityType(doc: BOEDocument): string {
   ...
   */
  private generateNotificationTitle(doc: BOEDocument, prompt: string): string {
